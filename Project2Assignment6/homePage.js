...
let  createHomePage=()=>{
	...
	for( $i=0; $i < $box.length; $i++){			
		...
		$link = 'https://www.omdbapi.com/?s='+$title+'&y=2018&apikey=499e294';
...
let pressSearchBtn=($param)=>{
		...
		$('.btn .glyphicon-search').click((e)=>{ //when search button is clicked
			...
			if($param==="id"){ //if dropdown item is id
				...
				console.log($id);
				if($test){
					$link = 'https://www.omdbapi.com/?i='+$id+'&apikey=499e294';
				...
				if($test){
					$link = 'https://www.omdbapi.com/?s='+$title+'&apikey=499e294';
					getDataByAjaxCall($link, $param);
				}  else {
					showErrorMessage("Please provide valid input.");
				}				
				...
				if($test1 && $test2){
					$link = 'https://www.omdbapi.com/?s='+$title+'&y='+$year+'&apikey=499e294';
					getDataByAjaxCall($link, $param);
				}  else {
					showErrorMessage("Please provide valid input.");
				}								
